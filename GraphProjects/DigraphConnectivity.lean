...
* `Digraph.Walk` (with accompanying pattern definitions
  `Digraph.Walk.nil'` and `Digraph.Walk.cons'`)

* `Digraph.Walk.IsTrail`, `Digraph.Walk.IsPath`, and `Digraph.Walk.IsCycle`.

* `Digraph.Path`

* `Digraph.Walk.map` and `Digraph.Path.map` for the induced map on walks,
  given an (injective) graph homomorphism.

* `Digraph.Reachable` for the relation of whether there exists
  a walk between a given pair of vertices

* `Digraph.Preconnected` and `Digraph.Connected` are predicates
...
* `Digraph.Subgraph.Connected` gives subgraphs the connectivity
  predicate via `Digraph.subgraph.coe`.

* `Digraph.ConnectedComponent` is the type of connected components of
  a given graph.

* `Digraph.IsBridge` for whether an edge is a bridge edge

## Main statements

* `Digraph.isBridge_iff_mem_and_forall_cycle_not_mem` characterizes bridge edges in terms of
...
variable (G : Digraph V) (G' : Digraph V') (G'' : Digraph V'')

/-- A walk is a sequence of adjacent vertices.  For vertices `u v : V`,
the type `walk u v` consists of all walks starting at `u` and ending at `v`.

We say that a walk *visits* the vertices it contains.  The set of vertices a
walk visits is `Digraph.Walk.support`.

See `Digraph.Walk.nil'` and `Digraph.Walk.cons'` for patterns that
can be useful in definitions since they make the vertices explicit. -/
inductive Walk : V → V → Type u
  | nil {u : V} : Walk u u
  | cons {u v w : V} (h : G.Adj u v) (p : Walk v w) : Walk u w
  deriving DecidableEq
#align simple_graph.walk Digraph.Walk

attribute [refl] Walk.nil

@[simps]
instance Walk.instInhabited (v : V) : Inhabited (G.Walk v v) := ⟨Walk.nil⟩
#align simple_graph.walk.inhabited Digraph.Walk.instInhabited
...
#align simple_graph.walk.nil' Digraph.Walk.nil'

/-- Pattern to get `Walk.cons` with the vertices as explicit arguments. -/
@[match_pattern]
abbrev cons' (u v w : V) (h : G.Adj u v) (p : G.Walk v w) : G.Walk u w := Walk.cons h p
#align simple_graph.walk.cons' Digraph.Walk.cons'
...
protected def copy {u v u' v'} (p : G.Walk u v) (hu : u = u') (hv : v = v') : G.Walk u' v' :=
  hu ▸ hv ▸ p
#align simple_graph.walk.copy Digraph.Walk.copy

@[simp]
theorem copy_rfl_rfl {u v} (p : G.Walk u v) : p.copy rfl rfl = p := rfl
#align simple_graph.walk.copy_rfl_rfl Digraph.Walk.copy_rfl_rfl
...
theorem copy_copy {u v u' v' u'' v''} (p : G.Walk u v)
  ...
  subst_vars
  rfl
#align simple_graph.walk.copy_copy Digraph.Walk.copy_copy

@[simp]
theorem copy_nil {u u'} (hu : u = u') : (Walk.nil : G.Walk u u).copy hu hu = Walk.nil := by
  subst_vars
  rfl
#align simple_graph.walk.copy_nil Digraph.Walk.copy_nil
...
theorem copy_cons {u v w u' w'} (h : G.Adj u v) (p : G.Walk v w) (hu : u = u') (hw : w = w') :
  ...
  subst_vars
  rfl
#align simple_graph.walk.copy_cons Digraph.Walk.copy_cons
...
theorem cons_copy {u v w v' w'} (h : G.Adj u v) (p : G.Walk v' w') (hv : v' = v) (hw : w' = w) :
  ...
  subst_vars
  rfl
#align simple_graph.walk.cons_copy Digraph.Walk.cons_copy

theorem exists_eq_cons_of_ne {u v : V} (hne : u ≠ v) :
    ∀ (p : G.Walk u v), ∃ (w : V) (h : G.Adj u w) (p' : G.Walk w v), p = cons h p'
  | nil => (hne rfl).elim
  | cons h p' => ⟨_, h, p', rfl⟩
#align simple_graph.walk.exists_eq_cons_of_ne Digraph.Walk.exists_eq_cons_of_ne

/-- The length of a walk is the number of edges/darts along it. -/
def length {u v : V} : G.Walk u v → ℕ
  | nil => 0
  | cons _ q => q.length.succ
#align simple_graph.walk.length Digraph.Walk.length
...
#align simple_graph.walk.append Digraph.Walk.append

/-- The reversed version of `Digraph.Walk.cons`, concatenating an edge to
the end of a walk. -/
def concat {u v w : V} (p : G.Walk u v) (h : G.Adj v w) : G.Walk u w := p.append (cons h nil)
#align simple_graph.walk.concat Digraph.Walk.concat

theorem concat_eq_append {u v w : V} (p : G.Walk u v) (h : G.Adj v w) :
    p.concat h = p.append (cons h nil) := rfl
#align simple_graph.walk.concat_eq_append Digraph.Walk.concat_eq_append
...
#align simple_graph.walk.get_vert Digraph.Walk.getVert

@[simp]
theorem getVert_zero {u v} (w : G.Walk u v) : w.getVert 0 = u := by cases w <;> rfl
#align simple_graph.walk.get_vert_zero Digraph.Walk.getVert_zero
...
#align simple_graph.walk.get_vert_of_length_le Digraph.Walk.getVert_of_length_le

@[simp]
theorem getVert_length {u v} (w : G.Walk u v) : w.getVert w.length = v :=
  w.getVert_of_length_le rfl.le
#align simple_graph.walk.get_vert_length Digraph.Walk.getVert_length
...
#align simple_graph.walk.adj_get_vert_succ Digraph.Walk.adj_getVert_succ

@[simp]
theorem cons_append {u v w x : V} (h : G.Adj u v) (p : G.Walk v w) (q : G.Walk w x) :
    (cons h p).append q = cons h (p.append q) := rfl
#align simple_graph.walk.cons_append Digraph.Walk.cons_append

@[simp]
theorem cons_nil_append {u v w : V} (h : G.Adj u v) (p : G.Walk v w) :
    (cons h nil).append p = cons h p := rfl
#align simple_graph.walk.cons_nil_append Digraph.Walk.cons_nil_append
...
#align simple_graph.walk.append_nil Digraph.Walk.append_nil

@[simp]
theorem nil_append {u v : V} (p : G.Walk u v) : nil.append p = p :=
  rfl
#align simple_graph.walk.nil_append Digraph.Walk.nil_append
...
theorem append_assoc {u v w x : V} (p : G.Walk u v) (q : G.Walk v w) (r : G.Walk w x) :
  ...
  | cons h p' ih =>
    dsimp only [append]
    rw [ih]
#align simple_graph.walk.append_assoc Digraph.Walk.append_assoc
...
theorem append_copy_copy {u v w u' v' w'} (p : G.Walk u v) (q : G.Walk v w)
  ...
  subst_vars
  rfl
#align simple_graph.walk.append_copy_copy Digraph.Walk.append_copy_copy

theorem concat_nil {u v : V} (h : G.Adj u v) : nil.concat h = cons h nil := rfl
#align simple_graph.walk.concat_nil Digraph.Walk.concat_nil

@[simp]
theorem concat_cons {u v w x : V} (h : G.Adj u v) (p : G.Walk v w) (h' : G.Adj w x) :
    (cons h p).concat h' = cons h (p.concat h') := rfl
#align simple_graph.walk.concat_cons Digraph.Walk.concat_cons
...
#align simple_graph.walk.append_concat Digraph.Walk.append_concat
...
#align simple_graph.walk.concat_append Digraph.Walk.concat_append
...
theorem exists_cons_eq_concat {u v w : V} (h : G.Adj u v) (p : G.Walk v w) :
  ...
  | cons h' p ih =>
    ...
    rw [concat_cons, hc]
#align simple_graph.walk.exists_cons_eq_concat Digraph.Walk.exists_cons_eq_concat
...
#align simple_graph.walk.exists_concat_eq_cons Digraph.Walk.exists_concat_eq_cons

@[simp]
theorem length_nil {u : V} : (nil : G.Walk u u).length = 0 := rfl
#align simple_graph.walk.length_nil Digraph.Walk.length_nil

@[simp]
theorem length_cons {u v w : V} (h : G.Adj u v) (p : G.Walk v w) :
    (cons h p).length = p.length + 1 := rfl
#align simple_graph.walk.length_cons Digraph.Walk.length_cons
...
theorem length_copy {u v u' v'} (p : G.Walk u v) (hu : u = u') (hv : v = v') :
  ...
  subst_vars
  rfl
#align simple_graph.walk.length_copy Digraph.Walk.length_copy
...
#align simple_graph.walk.length_append Digraph.Walk.length_append

@[simp]
theorem length_concat {u v w : V} (p : G.Walk u v) (h : G.Adj v w) :
    (p.concat h).length = p.length + 1 := length_append _ _
#align simple_graph.walk.length_concat Digraph.Walk.length_concat

theorem eq_of_length_eq_zero {u v : V} : ∀ {p : G.Walk u v}, p.length = 0 → u = v
  | nil, _ => rfl
#align simple_graph.walk.eq_of_length_eq_zero Digraph.Walk.eq_of_length_eq_zero
...
theorem exists_length_eq_zero_iff {u v : V} : (∃ p : G.Walk u v, p.length = 0) ↔ u = v := by
  ...
  · rintro rfl
    exact ⟨nil, rfl⟩
#align simple_graph.walk.exists_length_eq_zero_iff Digraph.Walk.exists_length_eq_zero_iff

@[simp]
theorem length_eq_zero_iff {u : V} {p : G.Walk u u} : p.length = 0 ↔ p = nil := by cases p <;> simp
#align simple_graph.walk.length_eq_zero_iff Digraph.Walk.length_eq_zero_iff
...
-- /-- Auxiliary definition for `Digraph.Walk.concatRec` -/
...
-- #align simple_graph.walk.concat_rec_aux Digraph.Walk.concatRecAux

-- /-- Recursor on walks by inducting on `Digraph.Walk.concat`.

-- This is inducting from the opposite end of the walk compared
-- to `Digraph.Walk.rec`, which inducts on `Digraph.Walk.cons`. -/
-- @[elab_as_elim]
-- def concatRec {u v : V} (p : G.Walk u v) : motive u v p :=
--   reverse_reverse p ▸ concatRecAux @Hnil @Hconcat p.reverse
-- #align simple_graph.walk.concat_rec Digraph.Walk.concatRec

-- @[simp]
-- theorem concatRec_nil (u : V) :
--     @concatRec _ _ motive @Hnil @Hconcat _ _ (nil : G.Walk u u) = Hnil := rfl
-- #align simple_graph.walk.concat_rec_nil Digraph.Walk.concatRec_nil
...
-- #align simple_graph.walk.concat_rec_concat Digraph.Walk.concatRec_concat

end ConcatRec

theorem concat_ne_nil {u v : V} (p : G.Walk u v) (h : G.Adj v u) : p.concat h ≠ nil := by
  cases p <;> simp [concat]
#align simple_graph.walk.concat_ne_nil Digraph.Walk.concat_ne_nil
...
theorem concat_inj {u v v' w : V} {p : G.Walk u v} {h : G.Adj v w} {p' : G.Walk u v'}
  ...
  | cons _ _ ih =>
    ...
    · rw [concat_cons, cons.injEq] at he
      ...
      exact ⟨rfl, rfl⟩
#align simple_graph.walk.concat_inj Digraph.Walk.concat_inj

/-- The `support` of a walk is the list of vertices it visits in order. -/
def support {u v : V} : G.Walk u v → List V
  | nil => [u]
  | cons _ p => u :: p.support
#align simple_graph.walk.support Digraph.Walk.support

/-- The `edges` of a walk is the list of edges it visits in order. -/
def edges {u v : V} : G.Walk u v → List (V × V)
  | nil => []
  | cons' u v _ h p => (u, v) :: p.edges

@[simp]
theorem support_nil {u : V} : (nil : G.Walk u u).support = [u] := rfl
#align simple_graph.walk.support_nil Digraph.Walk.support_nil

@[simp]
theorem support_cons {u v w : V} (h : G.Adj u v) (p : G.Walk v w) :
    (cons h p).support = u :: p.support := rfl
#align simple_graph.walk.support_cons Digraph.Walk.support_cons
...
#align simple_graph.walk.support_concat Digraph.Walk.support_concat
...
theorem support_copy {u v u' v'} (p : G.Walk u v) (hu : u = u') (hv : v = v') :
  ...
  subst_vars
  rfl
#align simple_graph.walk.support_copy Digraph.Walk.support_copy
...
#align simple_graph.walk.support_append Digraph.Walk.support_append

theorem support_ne_nil {u v : V} (p : G.Walk u v) : p.support ≠ [] := by cases p <;> simp
#align simple_graph.walk.support_ne_nil Digraph.Walk.support_ne_nil
...
#align simple_graph.walk.tail_support_append Digraph.Walk.tail_support_append

theorem support_eq_cons {u v : V} (p : G.Walk u v) : p.support = u :: p.support.tail := by
  cases p <;> simp
#align simple_graph.walk.support_eq_cons Digraph.Walk.support_eq_cons

@[simp]
theorem start_mem_support {u v : V} (p : G.Walk u v) : u ∈ p.support := by cases p <;> simp
#align simple_graph.walk.start_mem_support Digraph.Walk.start_mem_support

@[simp]
theorem end_mem_support {u v : V} (p : G.Walk u v) : v ∈ p.support := by induction p <;> simp [*]
#align simple_graph.walk.end_mem_support Digraph.Walk.end_mem_support

@[simp]
theorem support_nonempty {u v : V} (p : G.Walk u v) : { w | w ∈ p.support }.Nonempty :=
  ⟨u, by simp⟩
#align simple_graph.walk.support_nonempty Digraph.Walk.support_nonempty

theorem mem_support_iff {u v w : V} (p : G.Walk u v) : w ∈ p.support ↔ w = u ∨ w ∈ p.support.tail :=
  by cases p <;> simp
#align simple_graph.walk.mem_support_iff Digraph.Walk.mem_support_iff

theorem mem_support_nil_iff {u v : V} : u ∈ (nil : G.Walk v v).support ↔ u = v := by simp
#align simple_graph.walk.mem_support_nil_iff Digraph.Walk.mem_support_nil_iff
...
#align simple_graph.walk.mem_tail_support_append_iff Digraph.Walk.mem_tail_support_append_iff

@[simp]
theorem end_mem_tail_support_of_ne {u v : V} (h : u ≠ v) (p : G.Walk u v) : v ∈ p.support.tail := by
  obtain ⟨_, _, _, rfl⟩ := exists_eq_cons_of_ne h p
  simp
#align simple_graph.walk.end_mem_tail_support_of_ne Digraph.Walk.end_mem_tail_support_of_ne
...
#align simple_graph.walk.mem_support_append_iff Digraph.Walk.mem_support_append_iff

@[simp]
theorem subset_support_append_left {V : Type u} {G : Digraph V} {u v w : V}
    (p : G.Walk u v) (q : G.Walk v w) : p.support ⊆ (p.append q).support := by
  simp only [Walk.support_append, List.subset_append_left]
#align simple_graph.walk.subset_support_append_left Digraph.Walk.subset_support_append_left

@[simp]
theorem subset_support_append_right {V : Type u} {G : Digraph V} {u v w : V}
...
#align simple_graph.walk.subset_support_append_right Digraph.Walk.subset_support_append_right

theorem coe_support {u v : V} (p : G.Walk u v) : (p.support : Multiset V) = {u} + p.support.tail :=
  by cases p <;> rfl
#align simple_graph.walk.coe_support Digraph.Walk.coe_support
...
#align simple_graph.walk.coe_support_append Digraph.Walk.coe_support_append
...
#align simple_graph.walk.chain_adj_support Digraph.Walk.chain_adj_support

theorem chain'_adj_support {u v : V} : ∀ (p : G.Walk u v), List.Chain' G.Adj p.support
  | nil => List.Chain.nil
  | cons h p => chain_adj_support h p
#align simple_graph.walk.chain'_adj_support Digraph.Walk.chain'_adj_support

/-- Every edge in a walk's edge list is an edge of the graph.
It is written in this form (rather than using `⊆`) to avoid unsightly coercions. -/
theorem edges_subset_edgeSet {u v : V} :
    ∀ (p : G.Walk u v) ⦃e : V × V⦄, e ∈ p.edges → e ∈ G.edgeSet
  | cons h' p', e, h  => by
    cases h
    · exact h'
...
theorem edges_concat {u v w : V} (p : G.Walk u v) (h : G.Adj v w) :
    (p.concat h).edges = p.edges.concat (v, w) := by
  induction p <;> simp [*, concat_nil]
...
theorem edges_append {u v w : V} (p : G.Walk u v) (p' : G.Walk v w) :
    (p.append p').edges = p.edges ++ p'.edges := by
  induction p <;> simp [*]

theorem cons_map_snd_edges {u v : V} (p : G.Walk u v) : (u :: p.edges.map (·.snd)) = p.support := by
  induction p <;> simp! [*]

theorem map_snd_edges {u v : V} (p : G.Walk u v) : p.edges.map (·.snd) = p.support.tail := by
  simpa using congr_arg List.tail (cons_map_snd_edges p)

theorem map_fst_edges_append {u v : V} (p : G.Walk u v) :
    p.edges.map (·.fst) ++ [v] = p.support := by
  induction p <;> simp! [*]

theorem map_fst_darts {u v : V} (p : G.Walk u v) : p.edges.map (·.fst) = p.support.dropLast := by
  simpa! using congr_arg List.dropLast (map_fst_edges_append p)
#align simple_graph.walk.map_fst_darts Digraph.Walk.map_fst_darts

@[simp]
theorem length_support {u v : V} (p : G.Walk u v) : p.support.length = p.length + 1 := by
  induction p <;> simp [*]
#align simple_graph.walk.length_support Digraph.Walk.length_support

@[simp]
theorem length_edges {u v : V} (p : G.Walk u v) : p.edges.length = p.length := by
  induction p <;> simp [*]

theorem edges_fst_mem_support_of_mem_darts {u v : V} :
    ∀ (p : G.Walk u v) {e : V × V}, e ∈ p.edges → e.fst ∈ p.support
  | cons h p', d, hd => by
...
theorem edges_snd_mem_support_of_mem_darts {u v : V} :
    ∀ (p : G.Walk u v) {e : V × V}, e ∈ p.edges → e.snd ∈ p.support
  | cons h p', d, hd => by
    simp only [support_cons, edges_cons, List.mem_cons] at hd ⊢
    rcases hd with (rfl | hd)
    · simp
...
#align simple_graph.walk.is_trail Digraph.Walk.IsTrail
...
#align simple_graph.walk.is_path Digraph.Walk.IsPath
...
#align simple_graph.walk.is_path.to_trail Digraph.Walk.IsPath.isTrail

/-- A *circuit* at `u : V` is a nonempty trail beginning and ending at `u`. -/
structure IsCircuit {u : V} (p : G.Walk u u) extends IsTrail p : Prop where
  ne_nil : p ≠ nil
#align simple_graph.walk.is_circuit Digraph.Walk.IsCircuit

-- porting note: used to use `extends to_trail : is_trail p` in structure
protected lemma IsCircuit.isTrail (h : IsCircuit p) : IsTrail p := h.toIsTrail
#align simple_graph.walk.is_circuit.to_trail Digraph.Walk.IsCircuit.isTrail

/-- A *cycle* at `u : V` is a circuit at `u` whose only repeating vertex
is `u` (which appears exactly twice). -/
structure IsCycle {u : V} (p : G.Walk u u) extends IsCircuit p : Prop where
  support_nodup : p.support.tail.Nodup
#align simple_graph.walk.is_cycle Digraph.Walk.IsCycle
...
#align simple_graph.walk.is_cycle.to_circuit Digraph.Walk.IsCycle.isCircuit

theorem isTrail_def {u v : V} (p : G.Walk u v) : p.IsTrail ↔ p.edges.Nodup :=
  ⟨IsTrail.edges_nodup, fun h => ⟨h⟩⟩
#align simple_graph.walk.is_trail_def Digraph.Walk.isTrail_def
...
theorem isPath_copy {u v u' v'} (p : G.Walk u v) (hu : u = u') (hv : v = v') :
  ...
  subst_vars
  rfl
#align simple_graph.walk.is_path_copy Digraph.Walk.isPath_copy

theorem isCircuit_def {u : V} (p : G.Walk u u) : p.IsCircuit ↔ p.IsTrail ∧ p ≠ nil :=
  Iff.intro (fun h => ⟨h.1, h.2⟩) fun h => ⟨h.1, h.2⟩
#align simple_graph.walk.is_circuit_def Digraph.Walk.isCircuit_def
...
theorem isCircuit_copy {u u'} (p : G.Walk u u) (hu : u = u') :
  ...
  subst_vars
  rfl
#align simple_graph.walk.is_circuit_copy Digraph.Walk.isCircuit_copy
...
#align simple_graph.walk.is_cycle_def Digraph.Walk.isCycle_def
...
theorem isCycle_copy {u u'} (p : G.Walk u u) (hu : u = u') :
  ...
  subst_vars
  rfl
#align simple_graph.walk.is_cycle_copy Digraph.Walk.isCycle_copy

@[simp]
theorem IsTrail.nil {u : V} : (nil : G.Walk u u).IsTrail :=
  ⟨by simp [edges]⟩
#align simple_graph.walk.is_trail.nil Digraph.Walk.IsTrail.nil

theorem IsTrail.of_cons {u v w : V} {h : G.Adj u v} {p : G.Walk v w} :
    (cons h p).IsTrail → p.IsTrail := by simp [isTrail_def]
#align simple_graph.walk.is_trail.of_cons Digraph.Walk.IsTrail.of_cons
...
#align simple_graph.walk.cons_is_trail_iff Digraph.Walk.cons_isTrail_iff
...
theorem IsTrail.of_append_left {u v w : V} {p : G.Walk u v} {q : G.Walk v w}
  ...
  exact ⟨h.1⟩
#align simple_graph.walk.is_trail.of_append_left Digraph.Walk.IsTrail.of_append_left

theorem IsTrail.of_append_right {u v w : V} {p : G.Walk u v} {q : G.Walk v w}
    (h : (p.append q).IsTrail) : q.IsTrail := by
  rw [isTrail_def, edges_append, List.nodup_append] at h
  exact ⟨h.2.1⟩
#align simple_graph.walk.is_trail.of_append_right Digraph.Walk.IsTrail.of_append_right

attribute [ext] BEq

@[simp]
theorem List.count_prod_eq [DecidableEq V] (xs : List (V × V)) (e : V × V) :
    @List.count (V × V) instBEqProd e xs = @List.count (V × V) instBEq e xs := by
  congr!
  ext p q
  cases p
  cases q
...
#align simple_graph.walk.cons_is_path_iff Digraph.Walk.cons_isPath_iff

@[simp]
theorem isPath_iff_eq_nil {u : V} (p : G.Walk u u) : p.IsPath ↔ p = nil := by
  cases p <;> simp [IsPath.nil]
#align simple_graph.walk.is_path_iff_eq_nil Digraph.Walk.isPath_iff_eq_nil
...
#align simple_graph.walk.is_path.of_append_left Digraph.Walk.IsPath.of_append_left
...
#align simple_graph.walk.is_cycle.not_of_nil Digraph.Walk.IsCycle.not_of_nil
...
#align simple_graph.walk.is_path.length_lt Digraph.Walk.IsPath.length_lt
...
#align simple_graph.walk.take_until Digraph.Walk.takeUntil
...
#align simple_graph.walk.drop_until Digraph.Walk.dropUntil
...
#align simple_graph.walk.take_spec Digraph.Walk.take_spec

theorem mem_support_iff_exists_append {V : Type u} {G : Digraph V} {u v w : V}
...
#align simple_graph.walk.mem_support_iff_exists_append Digraph.Walk.mem_support_iff_exists_append
  ...
  · cases h
    ...
    · simp! only
      ...
      · subst h'
        simp
      · rw [edges_cons]
        simp only [List.count_prod_eq, List.count_cons]
        split_ifs with h''
        · simp at h''
          obtain ⟨rfl, rfl⟩ := h''
          exact (h' rfl).elim
        · simp only [List.count_prod_eq] at ih
          apply ih
...
theorem takeUntil_copy {u v w v' w'} (p : G.Walk v w) (hv : v = v') (hw : w = w')
  ...
  subst_vars
  rfl
#align simple_graph.walk.take_until_copy Digraph.Walk.takeUntil_copy
...
theorem dropUntil_copy {u v w v' w'} (p : G.Walk v w) (hv : v = v') (hw : w = w')
  ...
  subst_vars
  rfl
#align simple_graph.walk.drop_until_copy Digraph.Walk.dropUntil_copy
...
theorem support_takeUntil_subset {u v w : V} (p : G.Walk v w) (h : u ∈ p.support) :
  ...
  exact Or.inl hx
#align simple_graph.walk.support_take_until_subset Digraph.Walk.support_takeUntil_subset

theorem support_dropUntil_subset {u v w : V} (p : G.Walk v w) (h : u ∈ p.support) :
    (p.dropUntil u h).support ⊆ p.support := fun x hx => by
  rw [← take_spec p h, mem_support_append_iff]
  exact Or.inr hx
#align simple_graph.walk.support_drop_until_subset Digraph.Walk.support_dropUntil_subset

theorem edges_takeUntil_subset {u v w : V} (p : G.Walk v w) (h : u ∈ p.support) :
    (p.takeUntil u h).edges ⊆ p.edges := fun x hx => by
  rw [← take_spec p h, edges_append, List.mem_append]
  exact Or.inl hx

theorem edges_dropUntil_subset {u v w : V} (p : G.Walk v w) (h : u ∈ p.support) :
    (p.dropUntil u h).edges ⊆ p.edges := fun x hx => by
  rw [← take_spec p h, edges_append, List.mem_append]
  exact Or.inr hx
...
#align simple_graph.walk.length_take_until_le Digraph.Walk.length_takeUntil_le
...
theorem length_dropUntil_le {u v w : V} (p : G.Walk v w) (h : u ∈ p.support) :
  ...
  exact Nat.le.intro this
#align simple_graph.walk.length_drop_until_le Digraph.Walk.length_dropUntil_le
...
#align simple_graph.walk.is_trail.take_until Digraph.Walk.IsTrail.takeUntil
...
#align simple_graph.walk.is_trail.drop_until Digraph.Walk.IsTrail.dropUntil
...
#align simple_graph.walk.is_path.take_until Digraph.Walk.IsPath.takeUntil
...
#align simple_graph.walk.is_path.drop_until Digraph.Walk.IsPath.dropUntil

/-- Rotate a loop walk such that it is centered at the given vertex. -/
def rotate {u v : V} (c : G.Walk v v) (h : u ∈ c.support) : G.Walk u u :=
  (c.dropUntil u h).append (c.takeUntil u h)
#align simple_graph.walk.rotate Digraph.Walk.rotate
...
#align simple_graph.walk.support_rotate Digraph.Walk.support_rotate

theorem rotate_edges {u v : V} (c : G.Walk v v) (h : u ∈ c.support) :
    (c.rotate h).edges ~r c.edges := by
...
#align simple_graph.walk.is_trail.rotate Digraph.Walk.IsTrail.rotate
...
protected theorem IsCircuit.rotate {u v : V} {c : G.Walk v v} (hc : c.IsCircuit)
  ...
  · intro hn
    ...
    simp at hn'
#align simple_graph.walk.is_circuit.rotate Digraph.Walk.IsCircuit.rotate
...
#align simple_graph.walk.is_cycle.rotate Digraph.Walk.IsCycle.rotate
...
#align simple_graph.path Digraph.Path
...
#align simple_graph.path.is_path Digraph.Path.isPath

@[simp]
protected theorem isTrail {u v : V} (p : G.Path u v) : (p : G.Walk u v).IsTrail :=
  p.property.isTrail
#align simple_graph.path.is_trail Digraph.Path.isTrail

/-- The length-0 path at a vertex. -/
@[refl, simps]
protected def nil {u : V} : G.Path u u :=
  ⟨Walk.nil, Walk.IsPath.nil⟩
#align simple_graph.path.nil Digraph.Path.nil

/-- The length-1 path between a pair of adjacent vertices. -/
@[simps]
def singleton {u v : V} (h : G.Adj u v) (hne : u ≠ v): G.Path u v :=
  ⟨Walk.cons h Walk.nil, by simp [hne]⟩

theorem mk'_mem_edges_singleton {u v : V} (h : G.Adj u v) (hne : u ≠ v) :
    (u, v) ∈ (singleton h hne : G.Walk u v).edges := by simp [singleton]
...
theorem count_edges_eq_one [DecidableEq V] {u v : V} {p : G.Path u v} (e : V × V)
    (hw : e ∈ (p : G.Walk u v).edges) : (p : G.Walk u v).edges.count e = 1 := by
  simpa using List.count_eq_one_of_mem p.property.isTrail.edges_nodup hw
...
theorem not_mem_edges_of_loop {v : V} {e : V × V} {p : G.Path v v} : ¬e ∈ (p : G.Walk v v).edges :=
  by simp [p.loop_eq]
...
The result is a path, as shown in `Digraph.Walk.bypass_isPath`.
This is packaged up in `Digraph.Walk.toPath`. -/
def bypass {u v : V} : G.Walk u v → G.Walk u v
  ...
  | cons ha p =>
    ...
    if hs : u ∈ p'.support then
      ...
      cons ha p'
#align simple_graph.walk.bypass Digraph.Walk.bypass
...
#align simple_graph.walk.bypass_is_path Digraph.Walk.bypass_isPath
...
#align simple_graph.walk.length_bypass_le Digraph.Walk.length_bypass_le

/-- Given a walk, produces a path with the same endpoints using `Digraph.Walk.bypass`. -/
def toPath {u v : V} (p : G.Walk u v) : G.Path u v :=
  ⟨p.bypass, p.bypass_isPath⟩
#align simple_graph.walk.to_path Digraph.Walk.toPath
...
theorem support_bypass_subset {u v : V} (p : G.Walk u v) : p.bypass.support ⊆ p.support := by
  ...
  | cons _ _ ih =>
    ...
    · rw [support_cons]
      ...
      assumption
#align simple_graph.walk.support_bypass_subset Digraph.Walk.support_bypass_subset

theorem support_toPath_subset {u v : V} (p : G.Walk u v) :
    (p.toPath : G.Walk u v).support ⊆ p.support :=
  support_bypass_subset _
#align simple_graph.walk.support_to_path_subset Digraph.Walk.support_toPath_subset
...
theorem edges_bypass_subset {u v : V} (p : G.Walk u v) : p.bypass.edges ⊆ p.edges := by
  ...
  | cons _ _ ih =>
    simp! only
    split_ifs
    · apply List.Subset.trans (edges_dropUntil_subset _ _)
...
theorem map_copy (hu : u = u') (hv : v = v') :
  ...
  subst_vars
  rfl
#align simple_graph.walk.map_copy Digraph.Walk.map_copy
...
#align simple_graph.walk.map_id Digraph.Walk.map_id
...
theorem map_map : (p.map f).map f' = p.map (f'.comp f) := by
  ...
  | nil => rfl
  | cons _ _ ih => simp [ih]
#align simple_graph.walk.map_map Digraph.Walk.map_map
...
theorem map_eq_of_eq {f : G →g G'} (f' : G →g G') (h : f = f') :
  ...
  subst_vars
  rfl
#align simple_graph.walk.map_eq_of_eq Digraph.Walk.map_eq_of_eq

@[simp]
theorem map_eq_nil_iff {p : G.Walk u u} : p.map f = nil ↔ p = nil := by cases p <;> simp
#align simple_graph.walk.map_eq_nil_iff Digraph.Walk.map_eq_nil_iff

@[simp]
theorem length_map : (p.map f).length = p.length := by induction p <;> simp [*]
#align simple_graph.walk.length_map Digraph.Walk.length_map

theorem map_append {u v w : V} (p : G.Walk u v) (q : G.Walk v w) :
    (p.append q).map f = (p.map f).append (q.map f) := by induction p <;> simp [*]
#align simple_graph.walk.map_append Digraph.Walk.map_append

@[simp]
theorem support_map : (p.map f).support = p.support.map f := by induction p <;> simp [*]
#align simple_graph.walk.support_map Digraph.Walk.support_map

@[simp]
theorem edges_map : (p.map f).edges = p.edges.map (Prod.map f f) := by induction p <;> simp [*]
...
theorem map_isPath_of_injective (hinj : Function.Injective f) (hp : p.IsPath) :
  ...
  | cons _ _ ih =>
    ...
    cases hinj hf
    exact hp.2 hx
#align simple_graph.walk.map_is_path_of_injective Digraph.Walk.map_isPath_of_injective
...
#align simple_graph.walk.is_path.of_map Digraph.Walk.IsPath.of_map
...
#align simple_graph.walk.map_is_path_iff_of_injective Digraph.Walk.map_isPath_iff_of_injective
...
#align simple_graph.walk.map_is_trail_of_injective Digraph.Walk.map_isTrail_of_injective
...
#align simple_graph.walk.map_is_cycle_iff_of_injective Digraph.Walk.map_isCycle_iff_of_injective

alias map_isCycle_iff_of_injective ↔ _ map_isCycle_of_injective
#align simple_graph.walk.map_is_cycle_of_injective Digraph.Walk.map_isCycle_of_injective
...
theorem map_injective_of_injective {f : G →g G'} (hinj : Function.Injective f) (u v : V) :
  ...
  | cons _ _ ih =>
    ...
    | cons _ _ =>
      ...
      apply ih
      simpa using h.2
#align simple_graph.walk.map_injective_of_injective Digraph.Walk.map_injective_of_injective

/-- The specialization of `Digraph.Walk.map` for mapping walks to supergraphs. -/
@[reducible]
def mapLe {G G' : Digraph V} (h : G ≤ G') {u v : V} (p : G.Walk u v) : G'.Walk u v :=
  p.map (Hom.mapSpanningSubgraphs h)
#align simple_graph.walk.map_le Digraph.Walk.mapLe

@[simp]
theorem mapLe_isTrail {G G' : Digraph V} (h : G ≤ G') {u v : V} {p : G.Walk u v} :
    (p.mapLe h).IsTrail ↔ p.IsTrail :=
  map_isTrail_iff_of_injective Function.injective_id
#align simple_graph.walk.map_le_is_trail Digraph.Walk.mapLe_isTrail

alias mapLe_isTrail ↔ IsTrail.of_mapLe IsTrail.mapLe
#align simple_graph.walk.is_trail.of_map_le Digraph.Walk.IsTrail.of_mapLe
#align simple_graph.walk.is_trail.map_le Digraph.Walk.IsTrail.mapLe

@[simp]
theorem mapLe_isPath {G G' : Digraph V} (h : G ≤ G') {u v : V} {p : G.Walk u v} :
    (p.mapLe h).IsPath ↔ p.IsPath :=
  map_isPath_iff_of_injective Function.injective_id
#align simple_graph.walk.map_le_is_path Digraph.Walk.mapLe_isPath

alias mapLe_isPath ↔ IsPath.of_mapLe IsPath.mapLe
#align simple_graph.walk.is_path.of_map_le Digraph.Walk.IsPath.of_mapLe
#align simple_graph.walk.is_path.map_le Digraph.Walk.IsPath.mapLe

@[simp]
theorem mapLe_isCycle {G G' : Digraph V} (h : G ≤ G') {u : V} {p : G.Walk u u} :
    (p.mapLe h).IsCycle ↔ p.IsCycle :=
  map_isCycle_iff_of_injective Function.injective_id
#align simple_graph.walk.map_le_is_cycle Digraph.Walk.mapLe_isCycle

alias mapLe_isCycle ↔ IsCycle.of_mapLe IsCycle.mapLe
#align simple_graph.walk.is_cycle.of_map_le Digraph.Walk.IsCycle.of_mapLe
#align simple_graph.walk.is_cycle.map_le Digraph.Walk.IsCycle.mapLe
...
#align simple_graph.path.map Digraph.Path.map
...
#align simple_graph.path.map_injective Digraph.Path.map_injective

/-- Given a graph embedding, map paths to paths. -/
@[simps!]
protected def mapEmbedding (f : G ↪g G') {u v : V} (p : G.Path u v) : G'.Path (f u) (f v) :=
  Path.map f.toHom f.injective p
#align simple_graph.path.map_embedding Digraph.Path.mapEmbedding
...
#align simple_graph.path.map_embedding_injective Digraph.Path.mapEmbedding_injective
...
protected def transfer {u v : V} (p : G.Walk u v)
    (H : Digraph V) (h : ∀ e, e ∈ p.edges → e ∈ H.edgeSet) : H.Walk u v :=
  match p with
  | nil => nil
  | cons' u v w a p =>
    cons (h (u, v) (by simp)) (p.transfer H fun e he => h e (by simp [he]))
#align simple_graph.walk.transfer Digraph.Walk.transfer

variable {u v : V} (p : G.Walk u v)

theorem transfer_self : p.transfer G p.edges_subset_edgeSet = p := by
  induction p <;> simp [*]
#align simple_graph.walk.transfer_self Digraph.Walk.transfer_self

theorem transfer_eq_map_of_le (hp) (GH : G ≤ H) :
    p.transfer H hp = p.map (Digraph.Hom.mapSpanningSubgraphs GH) := by
  induction p <;> simp [*]
#align simple_graph.walk.transfer_eq_map_of_le Digraph.Walk.transfer_eq_map_of_le

@[simp]
theorem edges_transfer (hp) : (p.transfer H hp).edges = p.edges := by
  induction p <;> simp [*]
#align simple_graph.walk.edges_transfer Digraph.Walk.edges_transfer

@[simp]
theorem support_transfer  (hp) : (p.transfer H hp).support = p.support := by
  induction p <;> simp [*]
#align simple_graph.walk.support_transfer Digraph.Walk.support_transfer

@[simp]
theorem length_transfer (hp) : (p.transfer H hp).length = p.length := by
  induction p <;> simp [*]
#align simple_graph.walk.length_transfer Digraph.Walk.length_transfer
...
#align simple_graph.walk.is_path.transfer Digraph.Walk.IsPath.transfer
...
#align simple_graph.walk.is_cycle.transfer Digraph.Walk.IsCycle.transfer
...
@[simp]
theorem transfer_transfer (hp) {K : Digraph V} (hp') :
  ...
  | cons _ _ ih =>
    ...
    apply ih
#align simple_graph.walk.transfer_transfer Digraph.Walk.transfer_transfer
...
#align simple_graph.walk.transfer_append Digraph.Walk.transfer_append
...
@[reducible]
def toDeleteEdges (s : Set (V × V)) {v w : V} (p : G.Walk v w) (hp : ∀ e, e ∈ p.edges → ¬e ∈ s) :
...
@[simp]
theorem toDeleteEdges_nil (s : Set (V × V)) {v : V} (hp) :
    (Walk.nil : G.Walk v v).toDeleteEdges s hp = Walk.nil := rfl

@[simp]
theorem toDeleteEdges_cons (s : Set (V × V)) {u v w : V} (h : G.Adj u v) (p : G.Walk v w) (hp) :
...
This is an abbreviation for `Digraph.Walk.toDeleteEdges`. -/
abbrev toDeleteEdge (e : V × V) (p : G.Walk v w) (hp : e ∉ p.edges) :
    (G.deleteEdges {e}).Walk v w :=
  p.toDeleteEdges {e} (fun e' => by contrapose!; simp (config := { contextual := true }) [hp])

@[simp]
theorem map_toDeleteEdges_eq (s : Set (V × V)) {p : G.Walk v w} (hp) :
...
protected theorem IsPath.toDeleteEdges (s : Set (V × V))
    {p : G.Walk v w} (h : p.IsPath) (hp) : (p.toDeleteEdges s hp).IsPath :=
  h.transfer _

protected theorem IsCycle.toDeleteEdges (s : Set (V × V))
    {p : G.Walk v v} (h : p.IsCycle) (hp) : (p.toDeleteEdges s hp).IsCycle :=
  h.transfer _

@[simp]
theorem toDeleteEdges_copy (s : Set (V × V))
...
See `Digraph.reachable_iff_reflTransGen`. -/
def Reachable (u v : V) : Prop := Nonempty (G.Walk u v)
#align simple_graph.reachable Digraph.Reachable
...
#align simple_graph.reachable_iff_nonempty_univ Digraph.reachable_iff_nonempty_univ

protected theorem Reachable.elim {p : Prop} {u v : V} (h : G.Reachable u v)
    (hp : G.Walk u v → p) : p :=
  Nonempty.elim h hp
#align simple_graph.reachable.elim Digraph.Reachable.elim
...
#align simple_graph.reachable.elim_path Digraph.Reachable.elim_path

protected theorem Walk.reachable {G : Digraph V} {u v : V} (p : G.Walk u v) : G.Reachable u v :=
  ⟨p⟩
#align simple_graph.walk.reachable Digraph.Walk.reachable
...
#align simple_graph.reachable.trans Digraph.Reachable.trans
...
#align simple_graph.reachable_iff_refl_trans_gen Digraph.reachable_iff_reflTransGen

protected theorem Reachable.map {G : Digraph V} {G' : Digraph V'} (f : G →g G')
    (h : G.Reachable u v) : G'.Reachable (f u) (f v) :=
  h.elim fun p => ⟨p.map f⟩
#align simple_graph.reachable.map Digraph.Reachable.map

theorem Iso.reachable_iff {G : Digraph V} {G' : Digraph V'} {φ : G ≃g G'} {u v : V} :
    G'.Reachable (φ u) (φ v) ↔ G.Reachable u v :=
  ⟨fun r => φ.left_inv u ▸ φ.left_inv v ▸ r.map φ.symm.toHom, Reachable.map φ.toHom⟩
#align simple_graph.iso.reachable_iff Digraph.Iso.reachable_iff

theorem Iso.symm_apply_reachable {G : Digraph V} {G' : Digraph V'} {φ : G ≃g G'} {u : V}
    {v : V'} : G.Reachable (φ.symm v) u ↔ G'.Reachable v (φ u) := by
  rw [← Iso.reachable_iff, RelIso.apply_symm_apply]
#align simple_graph.iso.symm_apply_reachable Digraph.Iso.symm_apply_reachable
...
-- #align simple_graph.reachable_is_equivalence Digraph.reachable_is_equivalence

-- /-- The equivalence relation on vertices given by `Digraph.Reachable`. -/
-- def reachableSetoid : Setoid V := Setoid.mk _ G.reachable_is_equivalence
-- #align simple_graph.reachable_setoid Digraph.reachableSetoid

-- /-- A graph is preconnected if every pair of vertices is reachable from one another. -/
-- def Preconnected : Prop := ∀ u v : V, G.Reachable u v
-- #align simple_graph.preconnected Digraph.Preconnected

-- theorem Preconnected.map {G : Digraph V} {H : Digraph V'} (f : G →g H) (hf : Surjective f)
--     (hG : G.Preconnected) : H.Preconnected :=
--   hf.forall₂.2 fun _ _ => Nonempty.map (Walk.map _) <| hG _ _
-- #align simple_graph.preconnected.map Digraph.Preconnected.map

-- theorem Iso.preconnected_iff {G : Digraph V} {H : Digraph V'} (e : G ≃g H) :
...
-- #align simple_graph.iso.preconnected_iff Digraph.Iso.preconnected_iff
...
-- #align simple_graph.connected Digraph.Connected

-- instance : CoeFun G.Connected fun _ => ∀ u v : V, G.Reachable u v := ⟨fun h => h.preconnected⟩

-- theorem Connected.map {G : Digraph V} {H : Digraph V'} (f : G →g H) (hf : Surjective f)
--     (hG : G.Connected) : H.Connected :=
--   haveI := hG.nonempty.map f
--   ⟨hG.preconnected.map f hf⟩
-- #align simple_graph.connected.map Digraph.Connected.map

-- theorem Iso.connected_iff {G : Digraph V} {H : Digraph V'} (e : G ≃g H) :
--     G.Connected ↔ H.Connected :=
--   ⟨Connected.map e.toHom e.toEquiv.surjective, Connected.map e.symm.toHom e.symm.toEquiv.surjective⟩
-- #align simple_graph.iso.connected_iff Digraph.Iso.connected_iff

-- /-- The quotient of `V` by the `Digraph.Reachable` relation gives the connected
-- components of a graph. -/
-- def ConnectedComponent := Quot G.Reachable
-- #align simple_graph.connected_component Digraph.ConnectedComponent

-- /-- Gives the connected component containing a particular vertex. -/
-- def connectedComponentMk (v : V) : G.ConnectedComponent := Quot.mk G.Reachable v
-- #align simple_graph.connected_component_mk Digraph.connectedComponentMk
...
-- #align simple_graph.connected_component.inhabited Digraph.ConnectedComponent.inhabited

-- @[elab_as_elim]
-- protected theorem ind {β : G.ConnectedComponent → Prop}
--     (h : ∀ v : V, β (G.connectedComponentMk v)) (c : G.ConnectedComponent) : β c :=
--   Quot.ind h c
-- #align simple_graph.connected_component.ind Digraph.ConnectedComponent.ind
...
--   Quot.sound
-- #align simple_graph.connected_component.sound Digraph.ConnectedComponent.sound
...
-- #align simple_graph.connected_component.exact Digraph.ConnectedComponent.exact

-- @[simp]
-- protected theorem eq {v w : V} :
--     G.connectedComponentMk v = G.connectedComponentMk w ↔ G.Reachable v w :=
--   @Quotient.eq' _ G.reachableSetoid _ _
-- #align simple_graph.connected_component.eq Digraph.ConnectedComponent.eq
...
-- #align simple_graph.connected_component.connected_component_mk_eq_of_adj Digraph.ConnectedComponent.connectedComponentMk_eq_of_adj
...
-- #align simple_graph.connected_component.lift Digraph.ConnectedComponent.lift
...
--   rfl
-- #align simple_graph.connected_component.lift_mk Digraph.ConnectedComponent.lift_mk
...
-- #align simple_graph.connected_component.exists Digraph.ConnectedComponent.exists

-- protected theorem «forall» {p : G.ConnectedComponent → Prop} :
--     (∀ c : G.ConnectedComponent, p c) ↔ ∀ v, p (G.connectedComponentMk v) :=
--   (surjective_quot_mk G.Reachable).forall
-- #align simple_graph.connected_component.forall Digraph.ConnectedComponent.forall

-- theorem _root_.Digraph.Preconnected.subsingleton_connectedComponent (h : G.Preconnected) :
--     Subsingleton G.ConnectedComponent :=
--   ⟨ConnectedComponent.ind₂ fun v w => ConnectedComponent.sound (h v w)⟩
-- #align simple_graph.preconnected.subsingleton_connected_component Digraph.Preconnected.subsingleton_connectedComponent
...
-- #align simple_graph.connected_component.map Digraph.ConnectedComponent.map

-- @[simp]
-- theorem map_mk (φ : G →g G') (v : V) :
--     (G.connectedComponentMk v).map φ = G'.connectedComponentMk (φ v) :=
--   rfl
-- #align simple_graph.connected_component.map_mk Digraph.ConnectedComponent.map_mk

-- @[simp]
-- theorem map_id (C : ConnectedComponent G) : C.map Hom.id = C := by
--   refine' C.ind _
--   exact fun _ => rfl
-- #align simple_graph.connected_component.map_id Digraph.ConnectedComponent.map_id
...
--   refine' C.ind _
--   exact fun _ => rfl
-- #align simple_graph.connected_component.map_comp Digraph.ConnectedComponent.map_comp
...
-- #align simple_graph.connected_component.iso_image_comp_eq_map_iff_eq_comp Digraph.ConnectedComponent.iso_image_comp_eq_map_iff_eq_comp
...
-- #align simple_graph.connected_component.iso_inv_image_comp_eq_iff_eq_map Digraph.ConnectedComponent.iso_inv_image_comp_eq_iff_eq_map
...
-- #align simple_graph.iso.connected_component_equiv Digraph.Iso.connectedComponentEquiv
...
--   ext ⟨v⟩
--   rfl
-- #align simple_graph.iso.connected_component_equiv_refl Digraph.Iso.connectedComponentEquiv_refl
...
--   ext ⟨_⟩
--   rfl
-- #align simple_graph.iso.connected_component_equiv_symm Digraph.Iso.connectedComponentEquiv_symm
...
--   ext ⟨_⟩
--   rfl
-- #align simple_graph.iso.connected_component_equiv_trans Digraph.Iso.connectedComponentEquiv_trans
...
-- #align simple_graph.connected_component.supp Digraph.ConnectedComponent.supp
...
--   intro h
--   rw [reachable_comm, h]
-- #align simple_graph.connected_component.supp_injective Digraph.ConnectedComponent.supp_injective

-- @[simp]
-- theorem supp_inj {C D : G.ConnectedComponent} : C.supp = D.supp ↔ C = D :=
--   ConnectedComponent.supp_injective.eq_iff
-- #align simple_graph.connected_component.supp_inj Digraph.ConnectedComponent.supp_inj
...
--   Iff.rfl
-- #align simple_graph.connected_component.mem_supp_iff Digraph.ConnectedComponent.mem_supp_iff

-- theorem connectedComponentMk_mem {v : V} : v ∈ G.connectedComponentMk v :=
--   rfl
-- #align simple_graph.connected_component.connected_component_mk_mem Digraph.ConnectedComponent.connectedComponentMk_mem
...
-- #align simple_graph.connected_component.iso_equiv_supp Digraph.ConnectedComponent.isoEquivSupp
...
-- --   H.coe.Connected
-- -- #align simple_graph.subgraph.connected Digraph.Subgraph.Connected
...
-- --   subst_vars
-- --   rfl
-- -- #align simple_graph.singleton_subgraph_connected Digraph.singletonSubgraph_connected
...
-- -- #align simple_graph.subgraph_of_adj_connected Digraph.subgraphOfAdj_connected

-- theorem Preconnected.set_univ_walk_nonempty (hconn : G.Preconnected) (u v : V) :
--     (Set.univ : Set (G.Walk u v)).Nonempty := by
--   rw [← Set.nonempty_iff_univ_nonempty]
--   exact hconn u v
-- #align simple_graph.preconnected.set_univ_walk_nonempty Digraph.Preconnected.set_univ_walk_nonempty
...
-- #align simple_graph.connected.set_univ_walk_nonempty Digraph.Connected.set_univ_walk_nonempty
...
-- #align simple_graph.walk.to_subgraph Digraph.Walk.toSubgraph

-- theorem toSubgraph_cons_nil_eq_subgraphOfAdj (h : G.Adj u v) :
--     (cons h nil).toSubgraph = G.subgraphOfAdj h := by simp
-- #align simple_graph.walk.to_subgraph_cons_nil_eq_subgraph_of_adj Digraph.Walk.toSubgraph_cons_nil_eq_subgraphOfAdj
...
-- #align simple_graph.walk.to_subgraph_append Digraph.Walk.toSubgraph_append
...
-- #align simple_graph.walk.to_subgraph_reverse Digraph.Walk.toSubgraph_reverse
...
-- #align simple_graph.walk.to_subgraph_rotate Digraph.Walk.toSubgraph_rotate

-- @[simp]
-- theorem toSubgraph_map (f : G →g G') (p : G.Walk u v) : (p.map f).toSubgraph = p.toSubgraph.map f :=
--   by induction p <;> simp [*, Subgraph.map_sup]
-- #align simple_graph.walk.to_subgraph_map Digraph.Walk.toSubgraph_map
...
--     apply Set.toFinite
-- #align simple_graph.walk.finite_neighbor_set_to_subgraph Digraph.Walk.finite_neighborSet_toSubgraph
...
theorem set_walk_self_length_zero_eq (u : V) : {p : G.Walk u u | p.length = 0} = {Walk.nil} := by
  ext p
  simp
#align simple_graph.set_walk_self_length_zero_eq Digraph.set_walk_self_length_zero_eq
...
#align simple_graph.set_walk_length_zero_eq_of_ne Digraph.set_walk_length_zero_eq_of_ne
...
theorem set_walk_length_succ_eq (u v : V) (n : ℕ) :
  ...
  · simp only [Nat.succ_eq_add_one, Set.mem_setOf_eq, Walk.length_cons, add_left_inj,
    ...
    · rintro ⟨w, huw, pwv, rfl, rfl, rfl⟩
      rfl
#align simple_graph.set_walk_length_succ_eq Digraph.set_walk_length_succ_eq
...
-- See `Digraph.coe_finsetWalkLength_eq` for the relationship between this `Finset` and
...
-- #align simple_graph.finset_walk_length Digraph.finsetWalkLength
...
--     rw [← this]
-- #align simple_graph.coe_finset_walk_length_eq Digraph.coe_finsetWalkLength_eq
...
-- #align simple_graph.walk.mem_finset_walk_length_iff_length_eq Digraph.Walk.mem_finsetWalkLength_iff_length_eq
...
-- #align simple_graph.fintype_set_walk_length Digraph.fintypeSetWalkLength
...
-- #align simple_graph.set_walk_length_to_finset_eq Digraph.set_walk_length_toFinset_eq

-- /- See `Digraph.adjMatrix_pow_apply_eq_card_walk` for the cardinality in terms of the `n`th
...
--     rw [← Finset.mem_coe, coe_finsetWalkLength_eq]
-- #align simple_graph.card_set_walk_length_eq Digraph.card_set_walk_length_eq
...
-- #align simple_graph.fintype_set_path_length Digraph.fintypeSetPathLength
...
--   · rintro ⟨_, p, _⟩
--     exact ⟨p⟩
-- #align simple_graph.reachable_iff_exists_finset_walk_length_nonempty Digraph.reachable_iff_exists_finsetWalkLength_nonempty
...
-- #align simple_graph.reachable_delete_edges_iff_exists_walk Digraph.reachable_delete_edges_iff_exists_walk
...
-- #align simple_graph.is_bridge_iff_adj_and_forall_walk_mem_edges Digraph.isBridge_iff_adj_and_forall_walk_mem_edges
...
-- #align simple_graph.reachable_delete_edges_iff_exists_cycle.aux Digraph.reachable_deleteEdges_iff_exists_cycle.aux
...
-- #align simple_graph.adj_and_reachable_delete_edges_iff_exists_cycle Digraph.adj_and_reachable_delete_edges_iff_exists_cycle
...
-- #align simple_graph.is_bridge_iff_adj_and_forall_cycle_not_mem Digraph.isBridge_iff_adj_and_forall_cycle_not_mem
...
-- #align simple_graph.is_bridge_iff_mem_and_forall_cycle_not_mem Digraph.isBridge_iff_mem_and_forall_cycle_not_mem
